...
def onderhanden_werk_list(day=None, minimal_intesting_ohw_value: int = 0):
    ...
    def add_project_row(service_rows):
        row = service_rows[0]
        title = (
            f"{row['project_number']} - {row['organization']} - {row['project_name']}"
        )
        ...
        grid.add_row(
            [
                TextBlock(
                    title,
                    style=BOLD,
                    url="https://oberon.simplicate.com/projects/" + row["project_id"],
                ),
...
def render_onderhanden_werk_page(output_folder: Path):
    ...
    page = Page(
        [
            TextBlock(f'Onderhanden werk per {day.strftime("%d/%m")}', HEADER_SIZE),
            onderhanden_werk_list(day, minimal_intesting_ohw_value=1000),
        ]
    )
...
if __name__ == "__main__":
    ...
    days = [Day(), Day("2022-03-31")]
    for test_day in days:
        test_page = Page(
            [
                TextBlock(
                    f'Onderhanden werk per {test_day.strftime("%d/%m")}', HEADER_SIZE
                ),
                onderhanden_werk_list(test_day, minimal_intesting_ohw_value=1000),
            ]
        )
        if test_day == Day():
