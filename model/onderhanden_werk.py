...
def ohw_list(day: Day, minimal_intesting_value=0, group_by_project=0) -> DataFrame:
    ...
    service_turnovers = services_with_their_turnover(day, service_df)
...
@cache(hours=4)
def services_with_their_turnover(day: Day, service_df: DataFrame) -> dict[str, int]:
    timesheet = Timesheet()
    ...
    query = f'''select service_id, sum(turnover) as turnover 
                from timesheet 
                  ...
                  and day<"{day}"
               group by service_id'''
    result = {r['service_id']: int(r['turnover']) for r in timesheet.full_query(query)}
    return result
