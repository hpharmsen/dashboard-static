...
class Block:
    ...
    def __init__(
        ...
        for child in children:
            assert child is None or isinstance(
                child, Block
            ), f"Block {child} should be an instance of Block"
    ...
    def render_children(self):
        ...
        if self.align_children == "absolute":
            for left, top, child, link in self.children:
                rendered_child = child.do_render(
                    "absolute", options={"left": left, "top": top}
                )
                if link:
                    rendered_child = (
                        f'<a href="{link}" class="link">{rendered_child}</a>'
                    )
                res += "\n" + rendered_child
        else:
    ...
    def do_render(
        ...
        tooltip_text = (
            f'<span class="tooltiptext">{wrap(self.tooltip, 42)}</span>'
            if self.tooltip
            else ""
        )
...
class Grid(Block):
    ...
    def render_children(self):
        ...
        for row in range(self.rows):
            ...
            line_height = (
                f' style="line-height:{self.line_height}px;"'
                if self.line_height
                else ""
            )
